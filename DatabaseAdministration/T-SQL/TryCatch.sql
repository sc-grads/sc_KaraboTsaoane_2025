...
begin
	...
	begin try
		...
		select @NumOfEmployee = count(distinct EmployeeNumber) from tblEmployees
		where EmployeeNumber between @EmployeeNumberFrom and @EmployeeNumberTo
	...
	begin catch
		...
		SELECT ERROR_MESSAGE() AS ErrorMessage, ERROR_LINE() as ErrorLine, 
			   ...
			   --If the error occurred in a stored procedure, ERROR_LINE() returns the line number inside the procedure, not the whole script(Here is line 15)
			   -- 20-25 
