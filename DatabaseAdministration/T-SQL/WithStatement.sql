with tblWithRanking as   -- temporarily stores your subquery
...
select * from tblWithRanking   -- this is where you call the with Variable
where TheRank <= 5
...
(select D.Department, EmployeeNumber, EmployeeFirstName, EmployeeLastName,
       rank() over(partition by D.Department order by E.EmployeeNumber) as TheRank
 from tblDepartment as D 
 join tblEmployee as E on D.Department = E.Department),  --to create another one you just add a comma
Transaction2014 as
(select * from tblTransaction where DateOfTransaction < '2015-01-01')

select * from tblWithRanking     -- this is where you call the table
